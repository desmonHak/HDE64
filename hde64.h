...
/*
 * La tabla hde64_table probablemente contiene valores de configuración o propiedades específicas de 
 * cada instrucción (flags, tamaños de operando, modos de direccionamiento, etc.), organizados para 
 * un acceso eficiente. 
 *
 * 
 * hde64_table[hde64_table[opcode / 4] + (opcode % 4)]
 * 
 * opcode / 4 -> Divide el opcode entre 4, lo cual obtiene un índice base que apunta a un 
 *              grupo de 4 posibles valores asociados con ese opcode
 * 
 * Primer índice hde64_table[opcode / 4]: Utiliza el resultado anterior como índice en hde64_table. 
 *              Este valor sirve como un desplazamiento (offset) dentro de la tabla, apuntando al 
 *              bloque de valores relevantes para ese grupo de opcodes.
 * 
 * opcode % 4: Calcula el módulo de opcode con 4, lo cual da un índice de 0 a 3 que selecciona 
 *              la posición exacta dentro del bloque de 4 bytes.
 * 
 * hde64_table[... + (opcode % 4)]: Finalmente, se suma (opcode % 4) al desplazamiento obtenido 
 *              previamente, permitiendo acceder al valor específico de opcode dentro del bloque de 4 valores.
 * 
 * 
 * Ejemplo con instruccion syscall Opcode (0x0f 0x05)
 *  
 *      // si el primer opcode es 0x0f se o btiene un segundo opcode y se suma a ht
 *      // el valor constante de DELTA_OPCODES(0x4a) == 74
 *      if ((hs->opcode = c) == 0x0f) {
 ...
 *     }
 * 
 *  uint8_t temp0 = opcode / 4;   // temp0 = 5 / 4 = 1
 *  uint8_t temp1 = ht[temp0];    // temp1 = (DELTA_OPCODES + hde64_table)[1] == 0x8b
 *  uint8_t temp2 = (opcode % 4); // temp2 = 5 % 4 = 1
 *  // cflags = (DELTA_OPCODES + hde64_table)[0x8b + 1]  == hde64_table[430]
 *  cflags = ht[temp1 +
 *              // se divide entre 4 y se accede, luego se suma al
 *              // resto del resultado el opcode/4
 *              temp2];
...
unsigned char hde64_table[] = {
    ...
    0xc0, 0xc2, 0xc0, 0xe6, 0x03, 0x7f, 0x11, 0x7f, 0x01, 0x7f, 0x01, 0x3f, 0x01, 0x01, 
    // aqui empieza DELTA_OPCODES
                                                                                        0xab, // 75
    ...
    0x01, 0x01, 0x01, 0x01, 0xc4, 0x02, 0x10, 0xff, 
                                                    // 174
                                                    0xff, 0xff, 0x01, 0x00, 0x03, 0x11, 0xff, // 180
    ...
    0x04, 0x00, 0x02, 0xff, 0x00, 0xc0, 0xc2, 0x01, 0x01, 0x03, 0x03, 0x03, 0xca, 

    // aqui empieza DELTA_FPU_REG
                                                                                  0x40, 0x00, // 255
    0x0a, 0x00, 0x04, 0x00, 

    // Aqui empieza DELTA_FPU_MODRM(260)
                            0x00, 0x00, 0x00, 0x7f, 0x00, 0x33, 0x01, 0x00, 0x00, 0x00, 0x00, // 270
    ...
    // aqui empieza DELTA_PREFIXES(316)
    ...
    // aqui empieza DELTA_OP_LOCK_OK(430)
                                                                0x00, 0x00, 0x08, 0x00, 0x10, // 435
    0x00, 0x18, 0x00, 0x20, 0x00, 0x28, 0x00, 0x30, 0x00, 0x80, 0x01, 0x82, 0x01, 0x86, 0x00, // 450
    0xf6, 0xcf, 0xfe,

    // aqui empieza DELTA_OP2_LOCK_OK(454)
                     0x3f, 0xab, 0x00, 0xb0, 0x00, 0xb1, 0x00, 0xb3, 0x00, 0xba, 0xf8, 0xbb, // 465
    0x00, 0xc0, 0x00, 0xc1, 0x00, 0xc7, 

    // apartir de aqui, empieza DELTA_OP_ONLY_MEM(472)
                                        0xbf,0x62, 0xff, 0x00, 0x8d, 0xff, 0x00, 0xc4, 0xff, // 480
    0x00, 0xc5, 0xff, 0x00, 0xff, 0xff, 
    // aqui acaba DELTA_OP2_ONLY_MEM - DELTA_OP_ONLY_MEM

    // aqui empieza DELTA_OP_ONLY_MEM2(487)
                                         0xeb, 0x01,0xff, 0x0e, 0x12, 0x08, 0x00, 0x13, 0x09, // 495
    ...
    // los valores 0xff que hay aqui representados, se usen para detectar las instrucciones
    // invalidas
};
